...
namefile = 'names_for_matching.csv'
...
namef = open(namefile, 'wt', encoding='utf-8')
namewrite = csv.writer(namef, delimiter=',', quoting=csv.QUOTE_MINIMAL,
        lineterminator='\n')
namewrite.writerow(['FacebookID', 'earliestContact'])
...
for person in range(0, len(allDetails)):
  oldestDate=datetime.today().date() #just initializing
  curMessage=''
  ...
  if curDate<oldestDate: oldestDate=curDate
  ...
  for message in range(0, len(allDetails[person])):
    if allDetails[person][message][3].date() == curDate:
      ...
      if len(curMessage) > 0: curMessage = '\n' + curMessage
      if len(curMessage) == 0: alumInit = twoWay
      tempMessage = '(' + allNames[person][message] + \
	  ' @ ' + allDetails[person][message][3].strftime('%I:%M%p') + '): '
      tempMessage = tempMessage + str(allDetails[person][message][4])
      curMessage = tempMessage + curMessage
  ...
  writer.writerow(outBuf)
  namewrite.writerow([curPerson, oldestDate])
